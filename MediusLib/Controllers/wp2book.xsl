<xsl:transform version="2.0"
  ...
  <xsl:template match="/rss/channel">
    <xsl:element name="medius:book">
      ...
      <xsl:element name="medius:chapters">
            ...
            <xsl:for-each select='item[wp:post_type="post" and wp:status="publish"]'>
              ...
              <xsl:element name="medius:post">
                ...
                <xsl:attribute name="publishDate">
                  ...
                  <xsl:value-of select="substring(pubDate, 13, 4)"/>-<xsl:value-of select="$vMonths/name[@num=substring(current()/pubDate, 9, 3)]"/>-<xsl:value-of select="substring(pubDate, 6, 2)"/>T<xsl:value-of select="substring(pubDate, 18, 8)"/>Z
                </xsl:attribute>
